...
func Agent(flags *types.AgentFlags) {
	...
	logrus.SetReportCaller(*flags.Verbose)
	...
	serverUrl, err := url.Parse(*flags.ServerAddr)
	if err == nil && serverUrl != nil && serverUrl.Scheme == "https" {
		...
		host, _, err := net.SplitHostPort(*flags.ServerAddr)
		if err != nil {
	...
	for {
		...
		if serverUrl != nil && serverUrl.Scheme == "https" {
			*flags.ServerAddr = strings.Replace(*flags.ServerAddr, "https://", "wss://", 1)
			//websocket
			err = wsconnect(&tlsConfig, *flags.ServerAddr, *flags.SocksProxy, *flags.UserAgent)
		} else {
			if *flags.SocksProxy != "" {
				//suppose that scheme is socks:// or socks5://
				var proxyUrl *url.URL
				proxyUrl, err = url.Parse(*flags.SocksProxy)
				if err != nil {
				...
				if proxyUrl.Scheme == "socks" || proxyUrl.Scheme == "socks5" {
					...
					conn, err = sockDial(*flags.ServerAddr, proxyUrl.Host, proxyUrl.User.Username(), pass)
				} else {
			...
			if err == nil {
				if *flags.AcceptFingerprint != "" {
					tlsConfig.InsecureSkipVerify = true
					tlsConfig.VerifyPeerCertificate = func(rawCerts [][]byte, verifiedChains [][]*x509.Certificate) error {
						crtFingerprint := sha256.Sum256(rawCerts[0])
						crtMatch, err := hex.DecodeString(*flags.AcceptFingerprint)
						if err != nil {
